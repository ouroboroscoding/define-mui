...
export default class Results extends React.PureComponent {
	...
	constructor(props: ResultsProps) {
		...
		this.titles = [];
		const oTypes: Record<string, string | null> = {};
		const oOptions: optionsType = {};
		this.dynCallbacks = {};
		for(const k of this.fields) {
			...
			//	If it's a node
				...
				oTypes[k] = (props.tree.get(k) as Node).type();
			}
	...
	_calculateTotals(types: Record<string, string | null>, data: Record<string, any>[]) {
		...
		const oTypes: Record<string, string> = {};
	 ...
	 */
	_orderChange(event: React.MouseEvent<HTMLElement>) {
	 ...
	 */
	_pageChange(event: any, page: number) {
		this.setState({ page });
	}
